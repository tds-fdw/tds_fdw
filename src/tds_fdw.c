...
void tdsExplainForeignScan(ForeignScanState *node, ExplainState *es)
{
    ...
    if (es->verbose)
    {
        ...
        ExplainPropertyText("Use remote estimate", festate->use_remote_estimate, es);
        ExplainPropertyText("Local tuple estimate", festate->local_tuple_estimate, es);
        ExplainPropertyText("Row estimate method", festate->row_estimate_method, es);
    }
